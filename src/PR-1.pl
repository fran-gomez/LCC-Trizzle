...
% una posici贸n y almacena la columna en dicha posicion en la variable F.
obtener_columna(Tablero, N, R):-
    transpose(Tablero, TableroTransp),
    obtener_fila(TableroTransp, N, R).

% El predicado obtener_fila/3 recibe como par谩metro una lista de listas (un tablero),
% una posicion y almacena la fila en dicha posici贸n en la variable F.
...
% + Principal: Posici贸n de la columna principal en el tablero.
...
% El predicado cruzar/3 verifica que haya en la lista L el elemento x. En caso de ser as铆, se verifica
...
% + Principal: Posici贸n de la columna principal en la lista.
...
% El predicado rellenar/2 rellena los espacios vac韔s del tablero con
% mamushkas al azar de tama駉 1. Utiliza los predicados auxiliares
...
% matriz. Su funci髇 es pura y exclusivamente de testeo.
print_matrix([]).
